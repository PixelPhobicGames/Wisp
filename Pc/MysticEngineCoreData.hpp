...
static void UpdatePlayer(){
    ...
    if (MysticCoreData.Level_ID == 30){
        ...
        if (IsMouseButtonDown(MOUSE_LEFT_BUTTON) && !MysticPlayerOne.Rocket_trigger){
            MysticPlayerOne.Rocket_trigger = true;
 

            MysticPlayerOne.Click_x = GetMouseX();
            MysticPlayerOne.Click_y = GetMouseY();
    ...
    if (IsKeyPressed(KEY_SPACE) && MysticPlayerOne.Projectile_trigger == false && MysticPlayerOne.CanShoot){
    ...
    if (IsKeyDown(KEY_RIGHT) == false && IsKeyDown(KEY_LEFT) == false){
...
static void UpdateEnemy(){
    ...
    for (int i = 0; i < EMC; i++)
    {
        if (MysticEnemy[i].isalive){
            ...
            if (MysticPlayerOne.x >= 280 && IsKeyDown(KEY_RIGHT)){
                MysticEnemy[i].x -= 5;
            }
            if (MysticPlayerOne.x <= 50 && IsKeyDown(KEY_LEFT)){
                MysticEnemy[i].x += 5;
            }
